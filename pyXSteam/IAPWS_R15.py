...
def tc_ptrhocp_R15(p, T, rho, my, cp, cv):
    ...
    logger.debug(" - ρ_dash: {} ζ(T_R_dash,ρ_dash):{} ".format(rho_dash, zeta_TR_roh))

    delta_v_delta_T = wsp(p, T)
    delta_roh_delta_T = - roh**2 * delta_v_delta_T
    zeta_T_roh = delta_roh_delta_T * (p_ref / roh_ref) # page 11 section 3.1. ???

    delta_Xi_dash = rho_dash * (zeta_T_roh - zeta_TR_roh * (T_R_dash / T_dash)) # Page 12 EQ 23
    if delta_Xi_dash < 0:
        delta_Xi_dash = 0
